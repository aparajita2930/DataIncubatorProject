...
pipe=Pipeline([('pca',PCA()), ('scaled',StandardScaler()), ('svm_gaussian',svm.SVC(kernel='rbf',C=1,gamma=1,class_weight='balanced'))])

cval=[2**-9,2**-8,2**-7]

gval=[2**-9,2**-8,2**-7,2**-6,2**-5, 2**-4]

pca_val=[1,2,3,9,1046]

gs=GridSearchCV(pipe, dict(pca__n_components=pca_val, svm_gaussian__C=cval, svm_gaussian__gamma=gval), cv=10, verbose=100)
...
outfile="grid_gaussiansvm_cancer_search_scores_{0}.out".format(int(time.time()))
